...
/**
 * A virtual FaissIndex that serves as a proxy for Lucene's FP16 flat vectors reader. Used for two
 * Faiss HNSW cases where flat storage is skipped natively via IO_FLAG_SKIP_STORAGE, because Lucene's
 * own {@code .vec} file already holds an equivalent (or more precise) copy: {@code flat} +
 * {@code half_float} fields, and FLOAT fields quantized via {@code sq, bits:16} (Faiss's own
 * ScalarQuantizer already stores that data as FP16 natively, so the native copy would otherwise
 * duplicate the same precision-level data). This class bridges the two by installing itself as the
 * flat storage under {@link FaissHNSWIndex}, providing access to the FP16 reader for scoring.
 */
