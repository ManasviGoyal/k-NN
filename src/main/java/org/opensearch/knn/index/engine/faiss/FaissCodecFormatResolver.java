...
public class FaissCodecFormatResolver implements CodecFormatResolver {
    ...
    public KnnVectorsFormat resolve(
        ...
        // sq,16 stays FLOAT-only as a mapping option (see FaissSQEncoder), but its native index already
        // stores FP16 internally - use the FP16 auxiliary delegate here too so the two copies aren't
        // duplicating the same precision-level data at different sizes.
        final boolean useHalfFloatVectorFormat = vectorDataType == VectorDataType.HALF_FLOAT || resolvedSpec.isFP16QuantizedIndex();
        return useHalfFloatVectorFormat
            ? new NativeEngines990HalfFloatKnnVectorsFormat(approximateThreshold, nativeIndexBuildStrategyFactory)
            : new NativeEngines990KnnVectorsFormat(approximateThreshold, nativeIndexBuildStrategyFactory);
    }
