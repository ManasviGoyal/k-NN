...
public interface CodecFormatResolver {
     ...
     * user selects {@code method=flat} with {@code half_float} + x32 compression. Default implementation
     * delegates to {@link #resolve(String, KNNMethodContext, Map, int, int, ResolvedIndexSpec, VectorDataType)}
     * with a null spec, so engines that do not consume the compression level (e.g., Faiss) don't need to override.
     */
    default KnnVectorsFormat resolve(
        ...
        return resolve(field, methodContext, params, defaultMaxConnections, defaultBeamWidth, null, vectorDataType);
    }
