 ...
 */
public class KNNHalfFloatVectorValues extends KNNVectorValues<float[]> {
    KNNHalfFloatVectorValues(final KNNVectorValuesIterator vectorValuesIterator) {
        super(vectorValuesIterator);
    }

    @Override
    public float[] getVector() throws IOException {
        final float[] vector = VectorValueExtractorStrategy.extractHalfFloatVector(vectorValuesIterator);
        this.dimension = vector.length;
        this.bytesPerVector = vector.length * 2;
        return vector;
    }
