...
public final class FaissFieldStrategy implements EngineFieldStrategy {
    ...
    public FieldTypeConfig buildFieldTypeConfig(
        ...
        if (spec.isSQOneBit()) {
            ...
            // Marks the field as fp16-quantized (encoder: sq, bits:16, or the legacy no-bits path that
            // resolves to the same thing) so native-index-build code can detect it via FieldInfo
            // attributes alone and skip writing a redundant native flat copy - see
            // MemOptimizedNativeIndexBuildStrategy. Normalized to bits=16 explicitly even for the
            // legacy FULL_PRECISION case, since what matters downstream is "is this fp16", not the
            // exact resolved enum value.
