...
public abstract class BasePerFieldKnnVectorsFormat extends PerFieldKnnVectorsFormat {
    ...
    public KnnVectorsFormat getKnnVectorsFormatForField(final String field) {
        ...
        // Always use HalfFloatFlatVectorsFormat for half_float fields, for both Lucene and native engines
        if (mappedFieldType.getVectorDataType() == VectorDataType.HALF_FLOAT) {
            return new NativeEngines990KnnVectorsFormat(
                new HalfFloatFlatVectorsFormat(),
                getApproximateThresholdValue(),
