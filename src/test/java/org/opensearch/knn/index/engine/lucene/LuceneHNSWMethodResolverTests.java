...
public class LuceneHNSWMethodResolverTests extends KNNTestCase {
    ...
    public void testResolveMethod_whenValid_thenResolve() {
        ...
        knnMethodContext = new KNNMethodContext(
            ...
            new MethodComponentContext(
                METHOD_HNSW,
                Map.of(METHOD_ENCODER_PARAMETER, new MethodComponentContext(ENCODER_SQ, Map.of(LUCENE_SQ_BITS, 7)))
            )
        );
        resolvedMethodContext = TEST_RESOLVER.resolveMethod(
            knnMethodContext,
    ...
    public void testResolveMethod_whenInvalid_thenThrow() {
        ...
        // Expect invalid when bits and compression level do not align
        expectThrows(
            ValidationException.class,
            () -> TEST_RESOLVER.resolveMethod(
                new KNNMethodContext(
    ...
    /**
     * Only x1 is supported for HALF_FLOAT for now; every other level - including x16, which is added
     * in a follow-up once real quantization backs it - must be rejected, not silently ignored or
     * conflated with an encoder.
     */
